...
export default function SeasonTypesPage() {
  ...
  const fetchSeasonTypes = async () => {
    try {
      const res = await API.get(
        `/season-types?page=${pagination.currentPage}&limit=${pagination.limit}`
      );
      console.log(res.data.data);
      setSeasonTypes(res.data.data);
  ...
  const handleSave = async () => {
    try {
      if (editingId) {
        await API.post(`/season-types/${editingId}`, formValue, {
          headers: {
  ...
  const openEditModal = (seasonType: SeasonType) => {
    setFormValue({
      title: seasonType.title,
      image: seasonType.image,
    });
    setEditingId(seasonType._id);
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <FaPlus className="text-sm" /> Add Season Type
        </button>
      </div>
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex-shrink-0 h-10 w-10">
                      <Image
                        src={seasonType?.image}
                        alt={seasonType.title}
                        width={40}
                        height={40}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                    {new Date(seasonType.createdAt || "").toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-3">
                      <button
                        onClick={() => openEditModal(seasonType)}
      ...
      <AddModal
        ...
        title={editingId ? "Edit Season Type" : "Add Season Type"}
      >
