...
export default function GameTypesPage() {
  ...
  const fetchGameTypes = async () => {
    try {
      const res = await API.get(
        `/game-types?page=${pagination.currentPage}&limit=${pagination.limit}`
      );
  ...
  const handleSave = async () => {
    try {
      if (editingId) {
        await API.post(`/game-types/${editingId}`, formValue, {
          headers: {
  ...
  const openEditModal = (gameType: GameType) => {
    setFormValue({
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <FaPlus className="text-sm" /> Add Game Type
        </button>
      </div>
              ...
              {gameTypes?.map((gameType) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                    {new Date(gameType.createdAt || "").toLocaleDateString()}
                  </td>
      ...
      <AddModal
        ...
        title={editingId ? "Edit Game Type" : "Add Game Type"}
      >
